...
const
katwidzenia=70;
skroltekst:array[0..35] of string=(
  'POMYS£ NA GR ',
  'GRZEGORZ DROZD',
  '',
  '',
  ...
  '',
  'MUZYKA W MENU',
  'FATER (PRO-CREATION, THE ODOURS)',
  'WWW.PRO-CREATION.PL',
  'WWW.THEODOURS.NET',
  '',
  '',
  '',
  'T£O MUZYCZNE W GRZE I INTRO/OUTRO',
  'GRZEGORZ DROZD',
  '',
  '',
  '',
  ...
  '',
  '',
  '2007 ZOMBIE MASTAH PRODAKSHANS',
  'HTTP://GAD.ART.PL/',
  'GAD@GAD.ART.PL'
);

var
mat_spec: array[0..3] of GLFloat=(1.00, 1.00, 1.00, 1.0);
mat_1a: array[0..3] of GLFloat=(0.70, 0.70, 0.70, 1.0);
mat_1d: array[0..3] of GLFloat=(0.70, 0.70, 0.70, 1.0);
mat_1s: array[0..3] of GLFloat=(1.00, 1.00, 1.00, 1.0);

pos1: array[0..3] of GLFloat=(200.0,200.0,100.0, 1.0);
pos1win: array[0..3] of GLFloat=(-4200,2700,-3000, 1.0);
light_ka0: array[0..3] of GLFloat=(0.30, 0.30, 0.25, 1.0);
light_kd0: array[0..3] of GLFloat=(0.30, 0.30, 0.25, 1.0);
light_ks0: array[0..3] of GLFloat=(0.30, 0.30, 0.25, 1.0);

light_ka1: array[0..3] of GLFloat=(0.80, 0.80, 0.75, 1.0);
light_kd1: array[0..3] of GLFloat=(0.80, 0.80, 0.75, 1.0);
light_ks1: array[0..3] of GLFloat=(1.00, 1.00, 0.95, 1.0);
...
var a:integer; px:real;
begin
if margines=1 then sx:=sx-(length(s)-1)*sr //srodek
   else
   if margines=2 then sx:=sx-(length(s)-1)*sr*2; //prawy

px:=0;
glPushMatrix;
for a:=1 to length(s) do begin
    case s[a] of
       #48..#57:rysuj_litere2d(ord(s[a])-48, sx,sy,sr);
       #65..#90:rysuj_litere2d(ord(s[a])-55, sx,sy,sr);
       #97..#122:rysuj_litere2d(ord(s[a])-87, sx,sy,sr);
       ...
       'π','•':rysuj_litere2d(41, sx,sy,sr);
       'Ê','∆':rysuj_litere2d(42, sx,sy,sr);
       'Í',' ':rysuj_litere2d(43, sx,sy,sr);
       '≥','£':rysuj_litere2d(44, sx,sy,sr);
       'Ò','—':rysuj_litere2d(45, sx,sy,sr);
       'Û','”':rysuj_litere2d(46, sx,sy,sr);
       'ú','å':rysuj_litere2d(47, sx,sy,sr);
       'ü','è':rysuj_litere2d(48, sx,sy,sr);
       'ø','Ø':rysuj_litere2d(49, sx,sy,sr);
...
//---------------------------------------------------------------------------
procedure pokaz_element_obiekt(var ob:Tobiekt; element:integer; srodek:boolean; innatekstura:integer=-1; beztekstur: boolean=false; zniszczony:real=0; rrand:integer=0);
var v,f,g,o:integer;
begin
  if not beztekstur then begin
      if innatekstura=-1 then
         wlacz_teksture(ob.tex)
      else
         wlacz_teksture(innatekstura);
  ...
  if srodek then begin
      with ob.o do
      with ob.o.Groups[element] do begin
        glBegin(GL_TRIANGLES);
          for f := 0 to High(Faces) do
          begin
            for v := 0 to 2 do
            begin
              if Faces[f].UV[v] < Length(TexCoords) then glTexCoord2fv(@TexCoords[Faces[f].UV[v]]);
              if Faces[f].Normal[v] < Length(Normals) then glNormal3fv(@Normals[Faces[f].Normal[v]]);
              if Faces[f].XYZ[v] < Length(Vertices) then begin
                 if zniszczony>0 then RandSeed:=round(Vertices[Faces[f].XYZ[v]].x+Vertices[Faces[f].XYZ[v]].y+Vertices[Faces[f].XYZ[v]].z)+rrand;
                 glVertex3f(Vertices[Faces[f].XYZ[v]].x-srx+((random-0.5)*zniszczony),
                            Vertices[Faces[f].XYZ[v]].y-sry+((random-0.5)*zniszczony),
                            Vertices[Faces[f].XYZ[v]].z-srz+((random-0.5)*zniszczony) );
              end;
            end;
          end;
        glEnd;
  ...
  if zniszczony>0 then Randomize;

end;

//---------------------------------------------------------------------------
procedure rysuj_mapke;
var v,f,g,o, a,x,z, x1,z1,z11, zwz10, zwx10:integer;
...
begin
  ...
  glPushAttrib(GL_ALL_ATTRIB_BITS);
    glPushMatrix;
      ...
      for z:=0 to ziemia.wz-2 do begin
          z1:=z;
          z11:=z+1;
          glBegin(GL_QUAD_STRIP);
          for x:=0 to ziemia.wx-2 do begin
              x1:=x;
...
begin
  ...
  case gra.rodzajmisji of
    0:begin
      ...
      pisz2d('zginÍ≥o:'+inttostr(gra.zginelo), 480, height-10, 4,2);
  ...
  if cheaty.god then pisz2d('NIEZNISZCZALNOå∆', 5, height-25, 3.2);
  if cheaty.fuel then pisz2d('MAX PALIWA', 5, height-31, 3.2);
  if cheaty.weapon then pisz2d('MAX BRONI', 5, height-37, 3.2);
  if cheaty.load then pisz2d('MAX £ADOWNOåCI', 5, height-43, 3.2);
  if cheaty.time then pisz2d('MAX CZASU', 5, height-49, 3.2);
...
{  wartosc := gracz.paliwo/gracz.maxpaliwa*(height-120);
  ...
  for a:=0 to b do begin
      if 95+(a+1)*nz-2<=415 then begin//zeby nie wyswietlac ostatniego niepelnego kawalka
          if a<b then px:=0.4 else
...
//             glColor4f(1.0,py,0.1,px);
  ...
  glBegin(GL_QUADS);
     if (gracz.temp>=240) and (licz mod 20<=10) then glColor4f(0.6,0.0,0.0,0.7)
                                                else glColor4f(0.0,0.0,0.3,0.4);
     glVertex2f( 100, 10);
     glVertex2f( 110, 10);
  ...
  else glColor4f(1,0.2,0.1, 0.4);

  rysuj_ikone(0,140,60, 10);
  if wysgr<76 then begin
      glBegin(GL_LINES);
...
{    glBegin(GL_QUADS);
    ...
    glPointSize(1);
    if licz mod 20<=12 then jas:=(8-licz mod 20)/8
       else jas:=1;
//    if licz mod 20<=12 then begin
    glBegin(GL_POINTS);
    case gra.rodzajmisji of
      0:begin
        for a:=0 to high(pilot) do if pilot[a].jest and (pilot[a].nalotnisku>=0) then begin
            px:= radrx-((pilot[a].x-ziemia.px-gracz.x)/ziemia.wlk)*rx;
            py:=-radry+((pilot[a].z-ziemia.pz-gracz.z)/ziemia.wlk)*ry;
            if px<-radrx then px:=px+radrx*2
            else
            if px>radrx then px:=px-radrx*2;
            if py<-radry then py:=py+radry*2
            else
            if py>radry then py:=py-radry*2;

            j:=sqrt2(sqr(px)+sqr(py));
            if j<=radrx then begin
      ...
      1:begin
        for a:=0 to high(dzialko) do if dzialko[a].jest and not dzialko[a].rozwalone then begin
...
//    if licz mod 12<=8 then begin
        if licz mod 12<=4 then glPointSize(1)
                          else glPointSize(2);
        glBegin(GL_POINTS);
          for a:=0 to high(mysliwiec) do if mysliwiec[a].jest and not mysliwiec[a].zniszczony then begin
...
{    glcolor4f(0.7,1.0,1.0,0.4);
  ...
  if gracz.zyje then begin
     if (gracz.paliwo>0) and (gracz.paliwo<40) and (licz mod 50<=24) then begin
        glColor3f(1,0,0);
        pisz2d('PALIWO SI  KO—CZY!', width div 2, 60, 7,1);
     end else
     if (gracz.paliwo<=0) then begin
        glColor3f(1,0,0);
        pisz2d('KONIEC PALIWA', width div 2, 60, 7,1);
     end;

     if (licz mod 20<=10) then begin
         if (gracz.temp>=280) then begin
            glColor3f(1,1,0);
            pisz2d('TEMPERATURA KRYTYCZNA!', width div 2, 60, 7,1);
         end else
         if (gracz.temp>=240) then begin
            glColor3f(1,1,0);
     ...
     if (licz mod 20<=10) then begin
         if (gracz.y>=2800) then begin
            glColor3f(1,1,0);
            pisz2d('JESTEå ZA WYSOKO!', width div 2, 80, 7,1);
         end;
     end;

     if (gra.czas>0) and (gra.czas<60) and (licz mod 40<=20) then begin
        glColor3f(1,0,0);
        pisz2d('CZAS SI  KO—CZY!', width div 2, 90, 7,1);
     end;
  end;

     if (gracz.zlywsrodku) and (licz mod 20<=13) then begin
        glColor3f(1,0.2,0.1);
        pisz2d('ALARM! WR”G NA POK£ADZIE!', width div 2, 90, 9,1);
     end;


  if (gra.czas=0) then begin
     glColor3f(1,0,0);
     pisz2d('KONIEC CZASU', width div 2, 90, 7,1);
  end;

  if not gracz.zyje then begin
     glColor3f(1,0,0);
...
{  if gra.ilepilotow=0 then
  ...
  if (gracz.zyje and gracz.stoi and gracz.namatce) and (gra.pkt=0) then begin
  ...
  if (gra.moznakonczyc and ((gracz.stoi and gracz.namatce) or (not gracz.zyje))) then begin
...
{         glColor4f(0.0,0.1,0.0,0.7);
         ...
         pisz2d_otoczka(m, 40, gra.pozycjaYtekstuintro-20, 5,0,
                        0.1,0.8,0.1,0.8,
                        0.0,0.1,0.0,0.7 );
     end;
  end;
...
begin
  ...
  if intro.czas>=30 then begin
     glColor4f(0.2,1,0.3,0.7);
     if gra.jakiemisje<>2 then
        n:='MISJA '+inttostr(gra.planeta)+' ZAKO—CZONA!'
     else n:='MISJA '+inttostr(winieta.epizodmisja)+' ZAKO—CZONA!';
     a:=(intro.czas-30) div 2;
     if a>length(n) then a:=length(n);
     m:=copy(n,1,a);
     ...
     if intro.czas>=z then begin
         ...
         if gra.misjawypelniona then begin
            ...
            if gra.zycia>=1 then //n:=n+#13'PRZYGOTUJ SI  DO NAST PNEJ...'
               else n:=n+#13'NIESTETY ZGIN•£Eå...';


            pisz2d('PREMIA ZA POZOZSTA£Y CZAS:'#13' PKT: '+inttostr(gra.czas*4)+', KASA: '+inttostr(gra.czas), 40, 70, 5);
            if gra.rodzajmisji=0 then
               pisz2d('PREMIA ZA WZI CIE DODATKOWYCH PILOT”W:'#13' PKT: '+inttostr((gra.zabranych-gra.minimum)*350)+', KASA: '+inttostr((gra.zabranych-gra.minimum)*30), 40, 50, 5)
            else
            if gra.rodzajmisji=1 then
               pisz2d('PREMIA ZA ZNISZCZENIE DODATKOWYCH DZIA£EK:'#13' PKT: '+inttostr((gra.dzialekzniszczonych-gra.dzialekminimum)*350)+', KASA: '+inttostr((gra.dzialekzniszczonych-gra.dzialekminimum)*30), 40, 50, 5);
         ...
         if a>length(n) then a:=length(n);
         m:=copy(n,1,a);
         pisz2d_otoczka(m, 40, height-60, 5, 0,
                        0.1,0.8,0.1,0.8,
                        0.0,0.1,0.0,0.7 );
     end;
  end;
...
const tr=3.5; //7
var v,f,g,o, a,x,z, xod,xdo, zod,zdo, x1,z1,z11, zwz10, zwx10, xsr,zsr:integer;
mat_2a,mat_2d,mat_2s: array[0..3] of GLFloat;
obrx,obry,obrz, odl,r,gx1,gz1:real;
widocznosc2:integer;
begin
  if (ziemia.widac<=0) then exit;
...
//    glPolygonMode(GL_FRONT, GL_LINE);
    wlacz_teksture3d(17);
    z:=zod;
    while z<=zdo do begin

        if abs(z-zsr)<widocznosc2 then begin

          z1:=(z+zwz10) mod ziemia.wz;
          z11:=(z+1+zwz10) mod ziemia.wz;
          glBegin(GL_QUAD_STRIP);
          x:=xod;
          while x<=xdo do begin

             if sqrt2(sqr(abs(x-xsr))+sqr(abs(z-zsr)))<widocznosc2 then begin
                ...
                glVertex3f(x*ziemia.wlk+ziemia.px,
                           ziemia.pk[x1,z1].p,
                           z*ziemia.wlk+ziemia.pz);
...
//    glPolygonMode(GL_FRONT, GL_FILL);
    ...
    for z:=zod to zdo{ziemia.wz-2} do begin
        ...
        for x:=xod to xdo{ziemia.wx-2} do begin
            ...
            if ziemia.pk[x1,z1].scen then begin
               //glVertex3f(x*ziemia.wlk+ziemia.px,ziemia.pk[x1,z1].p,z*ziemia.wlk+ziemia.pz);
               glPushMatrix;
        ...
        for x:=xod to xdo{ziemia.wx-2} do begin
            ...
            if ziemia.pk[x1,z1].scen then begin
               ...
               glPushMatrix;
                 ...
                 glScalef(ziemia.pk[x1,z1].scen_rozm,ziemia.pk[x1,z1].scen_rozm,ziemia.pk[x1,z1].scen_rozm);
                 if ziemia.pk[x1,z1].scen_rodz in [1,5] then begin
...
const tr=3.5; //7
var a,x,z, xod,xdo, zod,zdo, x1,z1,z11, zwz10, zwx10:integer;
mat_2a,mat_2d,mat_2s: array[0..3] of GLFloat;
obrx,obry,obrz, odl,r,gx1,gz1:real;
begin
  if not ustawienia.krzaki then exit;
  ...
  zwx10:=ziemia.wx*10;
    ...
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
      ...
      for z:=zod to zdo do begin
          z1:=(z+zwz10) mod ziemia.wz;
          for x:=xod to xdo do begin
              x1:=(x+zwx10) mod ziemia.wx;

              if length(ziemia.pk[x1,z1].krzaki)>0 then begin
                 for a:=0 to high(ziemia.pk[x1,z1].krzaki) do
                     if ziemia.pk[x1,z1].krzaki[a].jest then begin
  {                   odl:=sqrt2(sqr( ziemia.pk[x1,z1].krzaki[a].x-gracz.x) +
                      ...
                      if (ziemia.pk[x1,z1].krzaki[a].x>-ziemia.px / 2) and (gra.jestkamera[0,0]<ziemia.px / 2) then gx1:=gra.jestkamera[0,0]-ziemia.px*2
                         else if (ziemia.pk[x1,z1].krzaki[a].x<ziemia.px / 2) and (gra.jestkamera[0,0]>-ziemia.px / 2) then gx1:=gra.jestkamera[0,0]+ziemia.px*2
                         else gx1:=gra.jestkamera[0,0];
                      if (ziemia.pk[x1,z1].krzaki[a].z>-ziemia.pz / 2) and (gra.jestkamera[0,2]<ziemia.pz / 2) then gz1:=gra.jestkamera[0,2]-ziemia.pz*2
                         else if (ziemia.pk[x1,z1].krzaki[a].z<ziemia.pz / 2) and (gra.jestkamera[0,2]>-ziemia.pz / 2) then gz1:=gra.jestkamera[0,2]+ziemia.pz*2
                         else gz1:=gra.jestkamera[0,2];

                     odl:=sqrt2(sqr( ziemia.pk[x1,z1].krzaki[a].x-gx1) +
                                sqr( ziemia.pk[x1,z1].krzaki[a].y-gra.jestkamera[0,1]) +
                                sqr( ziemia.pk[x1,z1].krzaki[a].z-gz1) );

                     r:=((widocznosckrzak*ziemia.wlk)-odl)/(ziemia.wlk*5);
                     if r>1 then r:=1;

                     if r>0 then begin
                         glColor4f(ziemia.pk[x1,z1].krzaki[a].kr,ziemia.pk[x1,z1].krzaki[a].kg,ziemia.pk[x1,z1].krzaki[a].kb, r);
                         ...
                         glPushMatrix;
                           ...
                           glScalef(ziemia.pk[x1,z1].krzaki[a].rozm,ziemia.pk[x1,z1].krzaki[a].rozm,ziemia.pk[x1,z1].krzaki[a].rozm);
                           obrx:=ziemia.pk[x1,z1].krzaki[a].obrx;//+sin(x1+z1+licz/(153.5*ziemia.pk[x1,z1].krzaki[a].rozm))*10;
                           obrz:=-cos(x1+{-z1+}licz/(3*ziemia.pk[x1,z1].krzaki[a].rozm))*15;
                           obry:=ziemia.pk[x1,z1].krzaki[a].obry;//+cos(-x1-z1*3+licz/(244.3*ziemia.pk[x1,z1].krzaki[a].rozm))*90;
...
var
    right, up:array[0..2] of GLFloat;//={viewMatrix[0], viewMatrix[4], viewMatrix[8]};
...
begin
if (gracz.oslonablysk<=0) or (gracz.sila<=0.8) then exit;
...
glPushMatrix;
  ...
  if nazewnatrz then gluQuadricOrientation(dupa, GLU_OUTSIDE)
     else gluQuadricOrientation(dupa, GLU_INSIDE);
...
//    glScalef(5,5,5);
    ...
    for g := 0 to High(obiekt[ob_gracz].o.Groups) do begin
      with obiekt[ob_gracz].o.Groups[g] do begin
          glPushMatrix;
          glRotatef(gracz.elementy[g].obrx,1,0,0);
          glRotatef(gracz.elementy[g].obry,0,1,0);
          glRotatef(gracz.elementy[g].obrz,0,0,1);
          if g in [2..7] then pokaz_element_obiekt(obiekt[ob_gracz],g,false, -1, false, zni, gracz.randuszkodzenia)
             else begin
                  glPushMatrix;
                     glTranslatef(0,
                                  gracz.nacisk*5,
    ...
    if gracz.sila>0.4 then begin
        ...
        for g := 0 to High(obiekt[ob_gracz].o.Groups) do begin
          with obiekt[ob_gracz].o.Groups[g] do begin
              ...
              glRotatef(gracz.elementy[g].obry,0,1,0);
              glRotatef(gracz.elementy[g].obrz,0,0,1);
              if g in [2..7] then pokaz_element_obiekt(obiekt[ob_gracz],g,false,-1,true, zni, gracz.randuszkodzenia)
                 else begin
                      glPushMatrix;
                         glTranslatef(0,
                                      gracz.nacisk*5,
...
//    glDepthFunc(GL_LEQUAL);
    ...
    if (not gracz.uszkodzenia[0]) and
       (((gra.etap=0) and (intro.scena=1) and (gracz.y>=matka.y+4.4)) or
       ((gra.etap=1) and (gracz.zyje))) then begin
        glPushAttrib(GL_ALL_ATTRIB_BITS);
          glPushMatrix;
...
glDisable(GL_LIGHT1);
if gracz.swiatlodol>0 then glDisable(GL_LIGHT2);
...
if gracz.swiatlodol>0 then glEnable(GL_LIGHT2);
if not gracz.uszkodzenia[0] then glEnable(GL_LIGHT1);
...
for a:=0 to high(smiec) do if smiec[a].jest then with smiec[a] do begin

    if x<xod then x1:=x-ziemia.px*2
       else if x>xdo then x1:=x+ziemia.px*2
            else x1:=x;
    if z<zod then z1:=z-ziemia.pz*2
       else if z>zdo then z1:=z+ziemia.pz*2
            else z1:=z;

    if (x1>=xod) and (x1<=xdo) and (z1>=zod) and (z1<=zdo) then begin
...
//    glDisable(GL_FOG);
    ...
    glMaterialf(GL_FRONT, GL_SHININESS, 1);
    for a:=0 to high(dym) do if dym[a].jest and (dym[a].rodz<>2) then with dym[a] do begin
        ...
        if ((x1>=xod) and (x1<=xdo) and (z1>=zod) and (z1<=zdo)) or (gra.etap<>1) then begin
            if rodz=0 then
...
for a:=0 to high(iskry) do
  if iskry[a].jest then with iskry[a] do begin
      glPushMatrix;
        glTranslatef(x,y,z);
...
const
    mata: array[0..3] of GLFloat=(1.00, 1.00, 0.80, 1.0);
var a:integer; b,c:real; kr,kg,kb:real;
kierl, posl, swi:wek;
...
begin
  if (matka.widac<=0) then exit;

  if ((gra.etap=0) and (intro.scena=1) and (gracz.y>=matka.y+2.4)) or
     (gra.etap=1) then jest_swiatlo:=true
     else jest_swiatlo:=false;

  glPushMatrix;
    glTranslatef(matka.x,matka.y+8,matka.z);

      if jest_swiatlo and (matka.otwarcie_drzwi>0) then begin
        glEnable(GL_LIGHT7);
        posl[0]:=380;
        posl[1]:=35;
        posl[2]:=0;
        ...
        kierl[2]:=0;
        normalize(kierl);
        glLightfv(GL_LIGHT7, GL_SPOT_DIRECTION, @kierl); //wektor kierunku swiecenia
  ...
  //       glFogf (GL_FOG_DENSITY, 0.0065);
      ...
      if (gra.etap<>1) then begin
          rysuj_swiatelko(-14.5,-0.9,6.5,
                          0.4, 1,1,0.9, 0.4+random*0.1);
          rysuj_swiatelko(-14.5,-0.9,-6.5,
                          0.4, 1,1,0.9, 0.4+random*0.1);
      end;

      if (gra.etap=1) and
         (gra.jestkamera[0,1]<=matka.y-40) then begin

          rysuj_swiatelko(-9,-2.8,7,
                          0.1, 1,1,0.9, 0.4+random*0.1);
          rysuj_swiatelko(-9,-2.8,-7,
                          0.1, 1,1,0.9, 0.4+random*0.1);
          rysuj_swiatelko(10.4,-2.8,0,
                          0.1, 1,1,0.9, 0.4+random*0.1);
      end;
    ...
    if matka.widac>=1 then begin
       ...
       if (gracz.zyje and gracz.stoi and gracz.namatce) or
          (gra.etap=0) then begin
          kr:=0;
          kg:=1;
          kb:=0;
       end else begin
       ...
       for a:=0 to 2 do begin
           rysuj_swiatelko(sin(b+(a*120)*pi180)*c,
                           -8,
                           cos(b+(a*120)*pi180)*c,
                           3,
                           kr,kg,kb,
...
for a:=0 to high(pilot) do
  if pilot[a].jest then with pilot[a] do begin
     if ((nalotnisku>=0) and (ziemia.widac>0)) or
        ((nalotnisku<0) and (matka.widac>0)) or zawszewidac then begin
        ...
        if (x1>=xod) and (x1<=xdo) and (z1>=zod) and (z1<=zdo) then begin
            ...
            glPushMatrix;
              ...
              if not zly then
                 ...
                 tx:=8;
              if (not stoi) or (ani<>0) then begin
                  with obiekt[ob_pilot].o do begin
                    case rodzani of
                      0:begin//biegnie
                        for g := 0 to High(Groups) do begin
...
for a:=0 to high(dzialko) do
  if dzialko[a].jest and not dzialko[a].rozwalone then with dzialko[a] do begin
    ...
    //        pokaz_obiekt(obiekt[ob_dzialko]);
            if (gracz.conamierzone=0) and (gracz.namierzone=a) then begin
               glPushMatrix;
                 wlacz_teksture(9);
                 glPushAttrib(GL_ALL_ATTRIB_BITS);
            ...
            glRotatef(-kier,0,1,0);
            if rodzaj=0 then glCallList(l_dzialkowieza)
                        else glCallList(l_dzialkowieza2);

            if rodzaj=0 then begin
               if namierza then rysuj_swiatelko(-2.2,7.6,2.8, 4-swiatlo/15, 0.3,1,0.4, 0.9-swiatlo/100)
                           else rysuj_swiatelko(-2.2,7.6,2.8, 2-swiatlo/70, 1,0.2,0.8, 0.7-swiatlo/160);
            end else begin
               if namierza then rysuj_swiatelko(2,6.6,5.5, 4-swiatlo/15, 0.3,1,0.4, 0.9-swiatlo/100)
                           else rysuj_swiatelko(2,6.6,5.5, 2-swiatlo/70, 1,0.2,0.8, 0.7-swiatlo/160);
            end;

            glRotatef(kat,1,0,0);
    //        pokaz_obiekt(obiekt[ob_dzialkolufa]);

            if rodzaj=0 then glCallList(l_dzialkolufa)
                        else glCallList(l_dzialkolufa2);
...
for a:=0 to high(rakieta) do
  if rakieta[a].jest then with rakieta[a] do begin
      ...
      glPushMatrix;
        ...
        case rodzaj of
          0:begin
             ...
             if paliwo>0 then rysuj_swiatelko(-2.4,0,0, 4+random, 1,1,0.8, 0.8+random*0.2);
            end;
          1:begin
             wylacz_teksture;
             glEnable(GL_COLOR_MATERIAL);
          ...
          2:begin
            for b:=0 to 1 do
             rysuj_swiatelko(0,0,0, 20+random*20-ord(paliwo<=0)*11, 1,0.9+random*0.1,0.8+random*0.2, 0.8+random*0.2-ord(paliwo<=0)*0.5);
            end;
        end;
      glPopMatrix;
...
for a:=0 to high(mysliwiec) do
  if mysliwiec[a].jest then with mysliwiec[a] do begin
      ...
      if (x1>=xod) and (x1<=xdo) and (z1>=zod) and (z1<=zdo) then begin
          ...
          glPushMatrix;
            ...
            if (gracz.conamierzone=1) and (gracz.namierzone=a) then begin
               glPushMatrix;
                 wlacz_teksture(9);
                 glPushAttrib(GL_ALL_ATTRIB_BITS);
                    glColor4f(1,0.2,0.2,1);
...
glPushMatrix;
  ...
  if not winieta.jest then begin
      ...
      if ztylu then begin
         if gra.rodzajmisji=0 then
            glTranslatef(1000+(random-0.5)*intro.czas2/50,
                         -70+(random-0.5)*intro.czas2/50,
      ...
      if ztylu then begin
         if (intro.czas2<550) or (gra.rodzajmisji<>0) then begin
             glPushMatrix;
         ...
         if (gra.etap=2) and (gra.rodzajmisji=0) and
            (intro.czas2>=200) and (intro.czas2<=650) then begin
            if (intro.czas2<=250) then j:=((intro.czas2-200)/50)
            else
            if (intro.czas2>500) then j:=((650-intro.czas2)/150)
            else j:=1;
            rysuj_swiatelko(400,0,0,
                            1400+j*2200+random*50, 1,1,0.8, j+random*0.04);
         end;
...
case gra.etap of
 0:begin//intro
   ...
   case intro.scena of
     ...
     0:gluLookAt(matka.x-1600+intro.czas2*20,
                 matka.y+200-intro.czas2*3,
                 matka.z-300-intro.czas2,

                 matka.x+intro.czas2/2,
                 matka.y+50+intro.czas2/15,
                 matka.z,
                 0,1,0);
     1:gluLookAt(gracz.x+sin(intro.czas*pi180)*(150+400/(1+intro.czas/30)),
                 gracz.y+500/(1+intro.czas/30),
                 gracz.z-cos(intro.czas*pi180)*(150+400/(1+intro.czas/70)),
                 gracz.x,gracz.y,gracz.z,
                 0,1,0);
   end;
   end;
 1:begin//gra
   if not gra.pauza then
       gluLookAt(gra.jestkamera[0,0],gra.jestkamera[0,1],gra.jestkamera[0,2],
                 gra.jestkamera[1,0],gra.jestkamera[1,1],gra.jestkamera[1,2],
                 gra.jestkamera[2,0],gra.jestkamera[2,1],gra.jestkamera[2,2])
       else begin
 ...
 2:begin//outro
   ...
   case intro.scena of
     ...
     0:gluLookAt(matka.x+sin(intro.czas2*0.1*pi180)*(550+1000/(1+intro.czas2/180)),
                 matka.y-200-500/(1+intro.czas2/60),
                 matka.z-cos(intro.czas2*0.1*pi180)*(550+1000/(1+intro.czas2/180)),
                 matka.x,matka.y,matka.z,
                 0,1,0);
     //kamera zza matki, a matka odlatuje na wprost
     1:gluLookAt(-2000+intro.czas2/8,
                 matka.y-intro.czas2/2-100,
                 matka.z+300+intro.czas2/2,

                 matka.x,matka.y,matka.z,
                 0,1,0);
     //kamera sprzed matki, za nia widac planete
     2:gluLookAt(matka.x-800+intro.czas2/2,
                 matka.y+150+intro.czas2/10,
...
var width,height,a,b1,b2,b:integer;
viewMatrix:array[0..15] of GLFloat;
s:string;
...
begin
   ...
   gluLookAt(sin(licz/143)*2,-cos(licz/109.2)*2,3000+cos(licz/120)*2,
             0,0,0,
             0,1,0);
  ...
  koltla[0]:=0;  koltla[1]:=0;  koltla[2]:=0;  koltla[3]:=0;
  ...
  case winieta.corobi of
    0:begin//winieta
      ...
      pisz2d('nowa gra: misja poczπtkowa: '+inttostr(winieta.planetapocz), 640 div 2, height-250, 8+ord(winieta.kursor=0),1);
      ...
      pisz2d('wyjúcie', 640 div 2, height-370, 8+ord(winieta.kursor=4),1);
      ...
      for a:=0 to high(skroltekst) do begin
          ...
          if (b1>=height-220) and (b1<=height-180)     then glColor4f(1.0,1.0,1.0,0.4 -((-b1+height-180)/40)*0.4 )
          else if (b1>=height-80 ) and (b1<=height-40) then glColor4f(1.0,1.0,1.0,0.0 +((-b1+height-40)/40)*0.4 )
    ...
    1,3:begin//zapis, odczyt
      ...
      pisz2d('WYJåCIE', 640 div 2, height-450, 8,1);
    ...
    2:begin//sklep
      ...
      if winieta.kursor<=6 then b1:=height-120-winieta.kursor*40
                           else b1:=height-412;
      glBegin(GL_QUADS);
        glColor4f(0.2, 0.2, 0.3, 0.4);
        glVertex2f(20,  b1);
      ...
      for a:=0 to 5 do begin
          if winieta.kursor=a then glColor4f(0.2,1.0,0.2,0.7) else glColor4f(0.1,0.6,0.1,0.6);
          case a of
            0:begin r:=1; s:='ZBIORNIK PALIWA'; end;
            1:begin r:=1; s:='RAKIETY'; end;
            2:begin r:=1; s:='DZIA£O MASZYNOWE'; end;
            3:begin r:=100; s:='OS£ONA'; end;
            4:begin r:=100; s:='UK£AD CH£ODZ•CY'; end;
            5:begin r:=1; s:='£ADOWNOå∆'; end;
          end;
//          pisz2d(s, 640 div 2, height-130-a*40, 8,1);
          ...
          if gra.poziomupgrade[a]<9 then begin
             if upgrade[a,gra.poziomupgrade[a]+1].cena<=gra.kasa then begin
                if winieta.kursor=a then glColor4f(0.2,1.0,0.2,0.7) else glColor4f(0.1,0.6,0.1,0.6);
             end else begin
                if winieta.kursor=a then glColor4f(1.0,0.2,0.2,0.7) else glColor4f(0.6,0.1,0.1,0.6);
             end;
             pisz2d('KUP: '+inttostr(round(upgrade[a,gra.poziomupgrade[a]+1].ile*r))+', CENA: '+inttostr(upgrade[a,gra.poziomupgrade[a]+1].cena), width-40, height-145-a*40, 6,2);
          end;
          ...
          for b:=0 to 9 do begin
              ...
              glBegin(GL_QUADS);
                ...
                if b<=gra.poziomupgrade[a] then glColor4f(0.2,0.6, 0.05, r)
                                           else glColor4f(0.3,0.05, 0.0, r);
                glVertex2f(b2+9, b1-5);
                glVertex2f(b2-8, b1-5);
              glEnd;
          end;
      ...
      if winieta.kursor=6 then glColor4f(0.2,1.0,0.2,0.7) else glColor4f(0.1,0.6,0.1,0.6);
      pisz2d('DODATKOWY L•DOWNIK', 40, height-370, 8,0);
      pisz2d('MASZ: '+inttostr(gra.zycia), 40, height-385, 6);
      if gra.zycia<9 then begin
         if cenazycia<=gra.kasa then begin
            if winieta.kursor=6 then glColor4f(0.2,1.0,0.2,0.7) else glColor4f(0.1,0.6,0.1,0.6);
         end else begin
            if winieta.kursor=6 then glColor4f(1.0,0.2,0.2,0.7) else glColor4f(0.6,0.1,0.1,0.6);
         end;
      ...
      for b:=1 to 9 do begin
          ...
          glBegin(GL_QUADS);
            ...
            glVertex2f(b2-8, b1+5);
            glVertex2f(b2+9, b1+5);
            if b<=gra.zycia then glColor4f(0.2,0.6, 0.05, r)
                            else glColor4f(0.3,0.05, 0.0, r);
            glVertex2f(b2+9, b1-5);
            glVertex2f(b2-8, b1-5);
          glEnd;
      ...
      pisz2d('WYJåCIE', 640 div 2, height-430, 8,1);
    ...
    4:begin//wybor misji dodatkowej
      ...
      if length(epizody)>=1 then begin
          b1:=0;
          if (high(epizody)>19) then begin
             b1:=winieta.epizod-10;
             if b1<0 then b1:=0;
             if b1+19>high(epizody) then b1:=high(epizody)-19;

          end;

          for a:=0 to 19 do begin
              if a+b1<=high(epizody) then begin
                 if winieta.epizod=a+b1 then glColor4f(0.2,1.0,0.2,0.7) else glColor4f(0.1,0.6,0.1,0.6);
                 pisz2d(inttostr(1+a+b1)+': '+epizody[a+b1].tytul+', MISJI:'+inttostr(length(epizody[a+b1].misje)), 50, height-70-a*15, 7);
              end;
          end;
      end else begin
      ...
      if gra.koniecgry then begin
         ...
         pisz2d('esc - powrÛt do winiety', 640 div 2, 30, 8,1);
      end;
...
const teksty:array[0..3] of record s:string; i:byte end=(
      (s:'ZOMBIE MASTAH PRODAKSHANS'; i:3),
      (s:'GRA GRZEGORZA DROZDA PT:'; i:2),
      (s:'RETROFIRE'; i:4),
      (s:'HTTP://GAD.ART.PL/'; i:2)
      );
  ilenapisow=4;
...
begin
  ...
  if glowneintro.czas>=(200+ilenapisow*300)-200 then
...
//    glDisable(GL_LIGHTING);
  ...
  glPushMatrix;
    ...
    for a:=0 to 39 do begin
        glPushMatrix;
          glTranslatef(random*200-100 ,
                       random*250-50 -glowneintro.czas/13,
  ...
  glPushMatrix;
    ...
    glDisable(GL_CULL_FACE);
       ...
       if (glowneintro.scena>=1) and (glowneintro.scena<=ilenapisow) then begin
          if glowneintro.czas2<=40 then r:= glowneintro.czas2/40
          else
          if glowneintro.czas2>=300-40 then r:= (300-glowneintro.czas2)/40
          else r:=1;
...
// glFogf(GL_FOG_DENSITY, 0.0065-ziemia.widac*0.005);
 ...
 glDisable(GL_LIGHTING);
  ...
  glPushMatrix;
     ...
     if kol[3]>1 then kol[3]:=1;
     if kol[3]<0 then kol[3]:=0;
...
begin
{
  ...
  if ((gra.etap=0) and (intro.scena=1) and (gracz.y>=matka.y+4.4)) or
     ((gra.etap=1) and (gracz.zyje)) then begin

     //reflektor
      if not gracz.uszkodzenia[0] and gracz.zyje then glEnable(GL_LIGHT1)
      ...
      glPushMatrix;
        ...
        normalize(kierl);
        glLightfv(GL_LIGHT1, GL_SPOT_DIRECTION, @kierl); //wektor kierunku swiecenia
     ...
     if gracz.swiatlodol>0 then begin
        ...
        kierl[1]:=-1;
        kierl[2]:=0;
        glLightfv(GL_LIGHT2, GL_SPOT_DIRECTION, @kierl); //wektor kierunku swiecenia
        glLightf(GL_LIGHT2, GL_SPOT_CUTOFF, 90); //obciecie stozka
        glLightf(GL_LIGHT2, GL_SPOT_EXPONENT, 2); //wygaszanie na brzegach swiatla
     end else
        glDisable(GL_LIGHT2);
  end else begin
      glDisable(GL_LIGHT1);
  ...
  //swiatla wybuchow
  if (gra.etap=1) then begin
     for a:=0 to 4 do glDisable(GL_LIGHT3+a);

     for a:=0 to high(swiatlo) do begin
         if swiatlo[a].jest then with swiatlo[a] do begin
            ...
            kierl[1]:=-1;
            kierl[2]:=0;
            glLightfv(GL_LIGHT3+a, GL_SPOT_DIRECTION, @kierl); //wektor kierunku swiecenia
            glLightf(GL_LIGHT3+a, GL_SPOT_CUTOFF, 90); //obciecie stozka
            glLightf(GL_LIGHT3+a, GL_SPOT_EXPONENT, 60-jasnosc*28); //wygaszanie na brzegach swiatla
...
if not glowneintro.jest then begin
    ...
    //  rysuj_slonce;
      ...
      if (gra.etap=2) and (intro.scena=2) then begin
         if (intro.czas2<350) or (gra.rodzajmisji<>0) then rysuj_planete(true,1)
         else
            if (gra.rodzajmisji=0) and
               ((intro.czas2>350) and (intro.czas2<650)) then rysuj_planete(true, (650-intro.czas2)/300 );
      end else
      if (gra.etap=0) and (intro.scena=0) then
...
{      if ((kamera<>7) or (gra.pauza)) and
      ...
      if ((gra.etap=0) and (intro.scena=1)) or
         ((gra.etap=1) and ((kamera<>7) or gra.pauza) and gracz.zyje)
         then rysuj_gracza
      else
      if (gra.etap=1) and (kamera=7) and (gracz.zyje) then rysuj_kokpit;
...
//         rysuj_krzaki;
    ...
    //    pos1win[3]:=0;
        ...
        glLightf(GL_LIGHT0, GL_SPOT_EXPONENT, 30); //wygaszanie na brzegach swiatla
        pokaz_winiete;
    end;
end else begin //glowne intro
