...
var
  texName: array[0..MAX_TEX_NAME] of integer;
...
{$I+}
  ...
  begin
    MessageBox(0, pchar('Cannot open file! ' + filename),
      'ERROR', MB_OK);
    Result := false;
    exit;
  end;
  ...
  begin
    MessageBox(0, pchar('TGA file be type 2 or type 10'#13#10'Is = ' +
...
begin
  Result := false;
  if not LoadTGA(tgaFile, nazwa) then
  begin
...
begin
  ...
  begin
    ...
    if not LoadTGA(tgaFile1, nazwa[A]) then
    begin
